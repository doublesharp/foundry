...
forgetest!(shared_compiler_cache_reuses_output_across_roots, |prj, cmd| {
    ...
    cmd.args(["build", "--allow-local-compiler"]).assert_success();
    ...
    second_cmd.args(["build", "--allow-local-compiler"]).assert_success();
    assert_eq!(
    ...
    second_cmd.args(["build", "--no-cache", "--allow-local-compiler"]).assert_success();
    assert_eq!(compiler_invocations(&invocations), 2, "--no-cache must bypass the shared cache");

    second.clear();
    second_cmd.forge_fuse().env("HOME", home.path());
    second_cmd.args(["build", "--force", "--allow-local-compiler"]).assert_success();
    ...
    first_cmd.args(["build", "--via-ir", "--allow-local-compiler"]).assert_success();
    ...
    second.clear();
    second_cmd.forge_fuse().env("HOME", home.path());
    second_cmd.args(["build", "--via-ir", "--allow-local-compiler"]).assert_success();
...
"#,
    ...
    second_cmd.forge_fuse().env("HOME", home.path());
    second_cmd.args(["build", "--allow-local-compiler"]).assert_failure();
    assert_eq!(
        compiler_invocations(&invocations),
        7,
